import { NavLink, Outlet } from 'react-router-dom';
import { useAuth } from '../context/AuthContext';
import { useIsMobile } from '../hooks/useIsMobile';
import '../App.css';

export function Layout() {
  const { user, logout } = useAuth();
  const isMobile = useIsMobile();

  if (isMobile) {
    return (
      <>
        <header className="mobile-header">
          <span className="logo">Бронирование</span>
          <div className="mobile-header-right">
            <span className="username">{user?.full_name}</span>
            <button className="btn-secondary btn-sm" onClick={logout}>Выйти</button>
          </div>
        </header>
        <main className="app-main app-main--mobile">
          <Outlet />
        </main>
        <nav className="bottom-nav">
          <NavLink to="/" end className="bottom-nav__item">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /></svg>
            <span>Календарь</span>
          </NavLink>
          <NavLink to="/today" className="bottom-nav__item">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2" /><circle cx="9" cy="7" r="4" /></svg>
            <span>Сегодня</span>
          </NavLink>
          <NavLink to="/my-bookings" className="bottom-nav__item">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 11l3 3L22 4" /><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11" /></svg>
            <span>Мои записи</span>
          </NavLink>
          {user?.role === 'admin' && (
            <>
              <NavLink to="/admin/users" className="bottom-nav__item">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 00-3-3.87" /><path d="M16 3.13a4 4 0 010 7.75" /></svg>
                <span>Юзеры</span>
              </NavLink>
              <NavLink to="/admin/settings" className="bottom-nav__item">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="3" /><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-4 0v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83-2.83l.06-.06A1.65 1.65 0 004.68 15a1.65 1.65 0 00-1.51-1H3a2 2 0 010-4h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 012.83-2.83l.06.06A1.65 1.65 0 009 4.68a1.65 1.65 0 001-1.51V3a2 2 0 014 0v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 2.83l-.06.06A1.65 1.65 0 0019.4 9a1.65 1.65 0 001.51 1H21a2 2 0 010 4h-.09a1.65 1.65 0 00-1.51 1z" /></svg>
                <span>Настройки</span>
              </NavLink>
            </>
          )}
        </nav>
      </>
    );
  }

  return (
    <>
      <header className="app-header">
        <span className="logo">Бронирование</span>
        <nav className="app-nav">
          <NavLink to="/" end>Календарь</NavLink>
          <NavLink to="/today">Сегодня</NavLink>
          <NavLink to="/my-bookings">Мои записи</NavLink>
          {user?.role === 'admin' && (
            <>
              <NavLink to="/admin/users">Пользователи</NavLink>
              <NavLink to="/admin/settings">Настройки</NavLink>
              <NavLink to="/admin/stats">Статистика</NavLink>
            </>
          )}
        </nav>
        <div className="app-header-right">
          <span className="username">{user?.full_name}</span>
          <NavLink to="/change-password" className="btn-secondary" style={{ fontSize: 12 }}>Пароль</NavLink>
          <button className="btn-secondary" onClick={logout}>Выйти</button>
        </div>
      </header>
      <main className="app-main">
        <Outlet />
      </main>
    </>
  );
}
